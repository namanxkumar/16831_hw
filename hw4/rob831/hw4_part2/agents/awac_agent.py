        ...
        if self.agent_params['discrete']:
            for i in range(self.agent_params['ac_dim']):
                candidate_action = torch.full(
                    (ob_no.shape[0],), i, device=ob_no.device, dtype=torch.long
                )
                q_vals_i = self.get_qvals(self.exploitation_critic, ob_no, candidate_action)
                probs_i = dist.probs[:, i]
                vals.append(probs_i * q_vals_i)
        else:
            
            for _ in range(n_actions):
                candidate_action = dist.sample()
                q_vals_i = self.get_qvals(self.exploitation_critic, ob_no, candidate_action)
                vals.append(q_vals_i)
        v_pi = torch.stack(vals, dim=0).mean(dim=0)
