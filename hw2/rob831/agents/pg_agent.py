...
class PGAgent(BaseAgent):
    ...
    def train(self, observations, actions, rewards_list, next_observations, terminals):
        ...
        # Compute q values and advantages, then update the policy
    ...
    def calculate_q_vals(self, rewards_list):
        ...
        if not self.reward_to_go:
            # Case 1: use the whole trajectory discounted return for each timestep
    ...
    def _discounted_return(self, rewards):
        ...
        # Full trajectory discounted return: sum_{t'=0}^{T-1} gamma^{t'} * r_{t'}
        # Each index t gets the same value (the total discounted return)
        total = 0
        for t in range(len(rewards)):
            total += (self.gamma ** t) * rewards[t]
        discounted_returns = np.array([total] * len(rewards))
    ...
    def _discounted_cumsum(self, rewards):
        ...
        # Reward-to-go: entry t = sum_{t'=t}^{T-1} gamma^{t'-t} * r_{t'}
        T = len(rewards)
